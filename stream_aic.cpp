...
static void reader_detect_mode()
{
    ...
    {
        aic_runtime.mode = (reader_mode_t)aic_cfg.reader.mode;
    }
...
int currentIndex;
unsigned char colorData[LED_COUNT * 3];

void ledUpdaterTask()
{
    WS2812 ledStrip(
        5,            // GP5
        LED_COUNT,         // Strip is 6 LEDs long.
    ...
    while (true)
    {
        ...
        colorData[currentIndex++] = c;
        //printf("\ngot: %d i: %d", c, currentIndex);

        if (currentIndex == (LED_COUNT * 3) - 1)
        {
            printf("\nupdating leds");
            for(int i = 0; i != LED_COUNT; i++) {
                unsigned char r = colorData[i * 3 + 0];
                unsigned char g = colorData[i * 3 + 1];
                unsigned char b = colorData[i * 3 + 2];

                ledStrip.setPixelColor(i, WS2812::RGB(r, g, b) );
            }
...
int main()
{
    ...
    multicore_launch_core1(ledUpdaterTask);
